...
foreach ( $parameters[ 'appointments' ] as $appointment ):
	...
	?>
	<tr data-id="<?php echo $appointment->id;?>" data-date="<?php echo $clientDate; ?>" data-date-original="<?php echo $originalDate; ?>" data-time-original="<?php echo $originalTime; ?>" data-time="<?php echo $clientTime; ?>" data-date-format="<?php echo $dateFormat; ?>" data-datebased="<?php echo ( int ) ($appointment->duration >= 24 * 60); ?>">
		<td>
			<div class="circle_image">
				<img src="<?php echo Helper::profileImage( $appointment->staff_profile_image, 'Staff');?>" alt="">
			</div>
		</td>
  ...
  <?php endif;?>
	<td class="appointment_info">
      <p><b><?php echo htmlspecialchars($appointment->service_name)?></b></p>
      <?php echo Helper::profileCard($appointment->staff_name, '', '', 'Staff');?>
	     <p><?php echo Helper::price( $appointment->total_price )?></p>
	     <p class="td_datetime"><?php echo $clientDate . ' ' . $clientTime; ?></p>
	     <p><b>Id: </b><?php echo htmlspecialchars($appointment->id)?></p>
	     <p class="booknetic_appointment_status_td"><span class="booknetic_appointment_status_all" style="color: <?php echo htmlspecialchars( $appointment->status_color ) ?>"><?php echo htmlspecialchars( $appointment->status_text ) ?></span></p>
		</td>
	...
	<td class="appointment_info">
			...
			<?php if( CustomerPanelHelper::canRescheduleAppointment( $appointment ) ): ?>
				<button class="booknetic_reschedule_btn" type="button" title="<?php echo bkntc__('Reschedule')?>"><i class="far fa-clock"></i>Reagendar</button>
				<?php else: ?>
        <button onclick="window.location.href='https://mitydi.com/booking';"  class="booknetic_repeat_btn" type="button" title="<?php echo bkntc__('Repeat') ?>"><i class="fa fa-refresh"></i> Repetir</button>
			<?php endif; ?>
   ...
   <?php if ( Helper::getOption('hide_pay_now_btn_customer_panel', 'off')=='off' && $appointment->total_price != $appointment->paid_amount && ! in_array( $appointment->status, [ 'canceled', 'rejected' ] ) ): ?>
       <button class="booknetic_pay_now_btn" type="button" title="<?php echo bkntc__('Pay Now')?>"><i class="fa fa-credit-card"></i>Pagar</button>
   <?php endif; ?>		 
		</td>
